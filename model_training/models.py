"""
Model training and evaluation script for survey data.
This script performs the following steps:
1. Load the survey data from a SQLite database.
2. Preprocess the data, including feature engineering and encoding.
3. Split the data into training and testing sets.
4. Train a Logistic Regression and Decision Tree model.
5. Evaluate the models using classification reports and confusion matrices.
6. Save the evaluation results and feature importance plots.
7. Save the processed data to a CSV file.
8. Save the confusion matrices and feature importance plots to a specified directory.
9. Return the paths to the saved files.
"""
# === Imports ===
import os
import sqlite3
...
# === Evaluation ===
report_lr = classification_report(y_test, y_pred_lr, target_names=target_le.classes_, output_dict=True)
report_dt = classification_report(y_test, y_pred_dt, target_names=target_le.classes_, output_dict=True)
...
plt.tight_layout()
conf_matrix_path = os.path.join(PLOTS_DIR, "confusion_matrices.png")
...
plt.tight_layout()
feat_importance_path = os.path.join(PLOTS_DIR, "feature_importance.png")
plt.savefig(feat_importance_path)
plt.close()

# Save processed data
PROCESSED_CSV = r"C:\Users\girli\OneDrive\Desktop\TTM_DS_Thesis\database\survey_processed_for_modeling01.csv"
